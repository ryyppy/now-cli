...
import type { NowError } from "../../../util/error";
const { createNowError, isNowError } = require("../../../util/error");
...
async function confirmDeploymentRemoval(_alias) {
  ...
  const tbl = table(
    [
      [...(_sourceUrl ? [_sourceUrl] : []), chalk.underline(_alias.alias), time]
    ],
    {
      align: ["l", "l", "r"],
      hsep: " ".repeat(4),
...
async function rm(ctx, opts, args, output): Promise<number> {
  ...
  try {
    const confirmation =
      opts["--yes"] || (await confirmDeploymentRemoval(alias));
    if (!confirmation) {
      log("Aborted");
      return 0;
    }
...
async function set(ctx, opts, args, output): Promise<number> {
  ...
  const params = await getTargetsAndDeployment(
    now,
    output,
    args,
    opts,
  ...
  if (isNowError(params)) {
    switch (params.code) {
      case "CANT_FIND_CONFIG":
        output.error(
          `Couldn't find a configuration file at \n    ${params.meta.paths.join(
            "\n    "
          )}.`
        );
        break;
      ...
      case "NO_ALIAS_IN_CONFIG":
        output.error(
          `Could't find an alias property into the given configuration.`
        );
        break;
      case "WRONG_ALIAS_IN_CONFIG":
        output.error(
          `Wrong value "${params.meta
            .value}" for alias found in config. It must be a string or array of string.`
        );
        break;
      case "CANT_FIND_A_DEPLOYMENT":
        output.error(
          `Could't find a deployment to alias for app '${params.meta
            .appName}'. Please specify one from the command line.`
        );
        break;
      case "DEPLOYMENT_NOT_FOUND":
        output.error(
          `Failed to find deployment "${params.meta.id}" under ${chalk.bold(
            contextName
          )}`
        );
        break;
      case "DEPLOYMENT_PERMISSION_DENIED":
        output.error(
          `No permission to access deployment "${params.meta
            .id}" under ${chalk.bold(contextName)}`
        );
        break;
      case "TOO_MANY_ARGS":
        output.error(
          `${cmd(
            "now alias <deployment> <target>"
  ...
  for (const target of addZeitDomainIfNeeded(targets)) {
    ...
    if (isNowError(result)) {
      switch (result.code) {
        case "DOMAIN_VERIFICATION_FAILED":
          ...
          output.print(
            `  Examples: (full list at ${chalk.underline(
              "https://zeit.world"
            )})\n`
          );
          output.print(
            `  ${chalk.gray("-")} ${chalk.underline(
              "a.zeit.world"
            )}    ${chalk.dim("96.45.80.1")}\n`
          );
          output.print(
            `  ${chalk.gray("-")} ${chalk.underline(
              "b.zeit.world"
            )}    ${chalk.dim("46.31.236.1")}\n`
          );
          output.print(
            `  ${chalk.gray("-")} ${chalk.underline(
              "c.zeit.world"
            )}    ${chalk.dim("43.247.170.1")}\n`
          );
          output.print(`\n  Alternatively, make sure to:`);
          output.print(
            `\n  ${chalk.gray(
              "-"
            )} Verify the domain by adding a TXT record on your DNS server: _now.${result
              .meta.domain}: ${result.meta.token}`
          );
          output.print(
            result.meta.subdomain === null
              ? `\n  ${chalk.gray("-")} Ensure it resolves to ${chalk.underline(
                  "alias.zeit.co"
                )} by adding an ${chalk.dim("ALIAS")} record for <domain>.\n`
              : `\n  ${chalk.gray("-")} Ensure it resolves to ${chalk.underline(
                  "alias.zeit.co"
                )} by adding a ${chalk.dim("CNAME")} record with name '${result
                  .meta.subdomain}'.\n`
          );
          break;
        case "UNABLE_TO_RESOLVE_EXTERNAL":
          ...
          output.print(
            result.meta.subdomain === null
              ? `\n  ${chalk.gray("-")} Ensure it resolves to ${chalk.underline(
                  "alias.zeit.co"
                )} by adding an ${chalk.dim("ALIAS")} record.\n`
              : `\n  ${chalk.gray("-")} Ensure it resolves to ${chalk.underline(
                  "alias.zeit.co"
                )} by adding a ${chalk.dim("CNAME")} record with name '${result
                  .meta.subdomain}'.\n`
        ...
        case "SOURCE_NOT_FOUND":
          output.error(
            `No credit cards found to buy ${result.meta
              .domain} – please run ${cmd("now cc add")}.`
          );
          break;
        case "NO_DOMAIN_PERMISSIONS":
          output.error(
            `You don't have permissions over domain ${result.meta
              .domain} under ${chalk.bold(contextName)}.`
          );
          break;
        case "USER_ABORT":
        ...
        case "DOMAIN_IS_NOT_VERIFIED":
          output.error(
            `We couldn't verify the domain ${result.meta
              .domain}. Please try again later`
          );
          break;
        case "ALIAS_IN_USE":
          output.error(
            `The alias ${chalk.dim(target)} is in use by a different team.`
          );
          break;
        case "DEPLOYMENT_NOT_FOUND":
          output.error(
            `Failed to find deployment "${result.meta.id}" under ${chalk.bold(
              contextName
            )}`
          );
          break;
        ...
        case "NEED_UPGRADE":
          output.error(
            `Custom domains are only supported for premium accounts. Please upgrade.`
          );
          break;
        case "DOMAIN_PERMISSION_DENIED":
          output.error(
            `You don't have permissions to access the domain ${target}`
          );
          break;
        ...
        case "DNS_ACCESS_UNAUTHORIZED":
          output.error(
            `You don't have permissions to access the DNS records for ${target}`
          );
          break;
        case "DEPLOYMENT_PERMISSION_DENIED":
          output.error(
            `No permission to access deployment "${result.meta
              .id}" under ${chalk.bold(contextName)}`
          );
          break;
        default:
    ...
    console.log(
      `${chalk.cyan("> Success!")} ${target} now points to ${chalk.bold(
        deployment.url
...
async function readConfigFromFile(
  file
): Promise<Config | null | ReadConfigErrors> {
...
async function readPackage(
  file?: string
): Promise<Package | null | ReadConfigErrors> {
...
async function readConfigFromPackage(
  file
): Promise<Config | null | ReadConfigErrors> {
  const pkg = await readPackage(file);
  if (isNowError(pkg) || pkg === null) {
    return pkg;
  }
...
async function getConfig(
  output,
  configFile
): Promise<Config | GetConfigErrors> {
  ...
  if (configFile) {
    ...
    if (localConfig || isNowError(localConfig)) {
      config = localConfig;
      return localConfig;
    }
  }
  ...
  if (mainConfig && !isNowError(mainConfig)) {
  ...
  if (pkgConfig && !isNowError(pkgConfig)) {
...
async function getAppName(output, opts): Promise<string> {
  ...
  if (!isNowError(config) && config.name) {
    return config.name;
  }

  // Otherwise try to get it from the package
  const pkg = await readPackage();
  if (!isNowError(pkg) && pkg !== null && pkg.name) {
    return pkg.name;
  }

  // Finally fallback to directory
  return path.basename(
    path.resolve(process.cwd(), opts["--local-config"] || "")
  );
}
...
type Deployment = NpmDeployment | StaticDeployment | BinaryDeployment;
...
async function fetchDeployment(
  ...
  contextName,
  id
): Promise<Deployment | FetchDeploymentErrors> {
  ...
  try {
    ...
    cancelWait();
    if (err.status === 404) {
      return createNowError({ code: "DEPLOYMENT_NOT_FOUND", meta: { id } });
...
async function fetchDeploymentsByAppName(
  now,
  appName
): Promise<Array<DeploymentItem>> {
...
async function getAppLastDeployment(
  ...
  user,
  contextName
): Promise<Deployment | GetAppLastDeploymentErrors> {
  output.debug(`Looking for deployments matching app ${appName}`);
  const cancelWait = wait(
    `Fetching user deployments in ${chalk.bold(contextName)}`
  );
  let deployments;
  try {
  ...
  return deploymentItem
    ...
    : createNowError({ code: "CANT_FIND_A_DEPLOYMENT", meta: { appName } });
}
...
async function getTargetsAndDeployment(
  now,
  output,
  args: Array<string>,
  opts,
  user,
  contextName
): Promise<SetAliasArgs | SetAliasErrors> {
  ...
  if (args.length === 0) {
    ...
    return !isNowError(deployment) ? { deployment, targets } : deployment;
  }
  ...
  if (args.length === 2) {
    ...
    return !isNowError(deployment)
      ? { deployment, targets: [target] }
      : deployment;
  }

  return createNowError({ code: "TOO_MANY_ARGS" });
}

type DomainValidationError = NowError<"INVALID_TARGET_DOMAIN",
  { target: string }>;
...
function addZeitDomainIfNeeded(targets) {
  return targets.map(target => {
    return target.indexOf(".") === -1 ? `${target}.now.sh` : target;
  });
}
...
function getSafeAlias(alias: string) {
  ...
  return _alias.indexOf(".") === -1 ? `${_alias}.now.sh` : _alias;
}
...
async function warnAliasOverwrite(
  output,
  alias: Alias
): Promise<true | UserAbortError> {
  if (isTTY) {
    const confirmed = await promptBool(chalk.bold.red("Are you sure?"));
    return !!confirmed || createNowError({ code: "USER_ABORT" });
  } else {
    output.log(
      `Overwriting path alias with ${plural(
        "rule",
        alias.rules.length,
        true
...
function getScaleForDC(
  dc: string,
  deployment: NpmDeployment | BinaryDeployment
): Scale {
...
function shouldCopyScalingAttributes(
  origin: NpmDeployment | BinaryDeployment,
  dest: NpmDeployment | BinaryDeployment
) {
  return (
...
async function setScale(output, now, deploymentId, scaleArgs) {
  ...
  output.success(
    `Scale rules for ${scalesMsg} saved ${elapsed(Date.now() - start)}`
  );
}
...
async function getDeploymentInstances(
  ...
  return now.fetch(
    `/v3/now/deployments/${encodeURIComponent(deploymentId)}/instances?init=1`
  );
}
...
async function matchDeploymentScale(
  output,
  now,
  deploymentId,
  scale
  ...
  for (const dc of dcsToScale) {
    ...
    if (isInstanceCountBetween(currentInstancesCount, min, max)) {
      ...
      output.debug(
        `DC ${dc} missing scale. Inteded (${min}, ${max}). Current ${currentInstancesCount}`
      );
    }
  }
...
function renderRemainingDCsWait(remainingDcs) {
  return wait(
    `Waiting for instances in ${Array.from(remainingDcs)
      .map(id => chalk.bold(id))
      .join(", ")} to match constraints`
  );
}
...
async function waitForScale(output, now, deploymentId, scale) {
  ...
  while (true) { // eslint-disable-line
    ...
    const matches = await matchDeploymentScale(
      output,
      now,
      deploymentId,
      scale
    ...
    const newMatches = new Set(
      [...remainingMatches].filter(dc => matches.has(dc))
    );
    remainingMatches = new Set(
      [...remainingMatches].filter(dc => !matches.has(dc))
    );
    ...
    if (newMatches.size !== 0) {
      ...
      for (const dc of newMatches) {
        // $FlowFixMe
        output.log(
          `${chalk.cyan(tick)} Scaled ${chalk.bold(dc)} (${matches.get(
            dc
          )} instance) ${elapsed(Date.now() - start)}`
        );
      }
...
type GetDomainErrors = NowError<"NO_DOMAIN_PERMISSIONS", { domain: string }>;

async function getDomainInfo(
  now,
  domain
): Promise<null | DomainInfo | GetDomainErrors> {
...
type GetDomainServersError = NowError<"NAMESERVERS_NOT_FOUND">;

async function getDomainNameservers(
  now,
  domain: string
): Promise<Array<string> | GetDomainServersError> {
  ...
  try {
    ...
    cancelFetchingMessage();
    if (error.status === 404) {
      return createNowError({ code: "NAMESERVERS_NOT_FOUND" });
    } else {
      throw error;
    }
  }
...
type BuyDomainErrors = NowError<"SOURCE_NOT_FOUND", { domain: string }>;

async function purchaseDomain(
  output,
  now,
  domain
): Promise<null | BuyDomainErrors> {
  ...
  try {
    ...
    if (error.code === "source_not_found") {
      return createNowError({ code: "SOURCE_NOT_FOUND", meta: { domain } });
    } else {
      throw error;
    }
  }
...
async function purchaseDomainIfAvailable(
  ...
  domain,
  contextName
): Promise<boolean | BuyDomainErrors> {
  ...
  if (available) {
    ...
    cancelWait();
    output.log(
      `The domain ${domain} is ${chalk.italic(
        "available"
      )} to buy under ${chalk.bold(contextName)}! ${buyDomainStamp()}`
    );

    if (
      !await promptBool(
        `Buy now for ${chalk.bold(`$${price}`)} (${plural(
          "yr",
          period,
          true
        )})?`
      ...
      return createNowError({ code: "USER_ABORT" });
    }
...
async function domainResolvesToNow(
  output,
  alias,
  retryConfig = {}
): Promise<boolean> {
...
| NowError<"NEED_UPGRADE", void>
| NowError<"DOMAIN_PERMISSION_DENIED", void>
  | NowError<"DOMAIN_IS_NOT_VERIFIED", { domain: string }>
  | NowError<"DOMAIN_VERIFICATION_FAILED",
...
async function verifyDomain(
  ...
  alias,
  { isExternal = false } = {}
): Promise<boolean | VerifyDomainErrors> {
  ...
  try {
    ...
    if (error.status === 403) {
      return error.code === "custom_domain_needs_upgrade"
        ? createNowError({ code: "NEED_UPGRADE" })
        : createNowError({ code: "DOMAIN_PERMISSION_DENIED" });
    }
...
type SetupDNSRecordError = NowError<"DNS_ACCESS_UNAUTHORIZED">;

async function setupDNSRecord(
  output,
  now,
  type,
  name,
  domain
): Promise<null | SetupDNSRecordError> {
  output.debug(
    `Trying to setup ${type} record with name ${name} for domain ${domain}`
  );
  try {
    ...
    if (err.status === 403) {
      return createNowError({ code: "DNS_ACCESS_UNAUTHORIZED", err });
    }
...
type UnableToResolveExternalError = NowError<"UNABLE_TO_RESOLVE_EXTERNAL",
...
type SetupAliasDomainError =
  ...
  | UserAbortError
  | UnableToResolveExternalError
  | SetupDNSRecordError;

async function setupAliasDomain(
  output,
  now,
  alias,
  contextName
): Promise<true | SetupAliasDomainError> {
  ...
  if (!info) {
    ...
    if (!nameservers.every(ns => ns.endsWith(".zeit.world"))) {
      ...
      if (!await domainResolvesToNow(output, alias, { retries: 10 })) {
        return createNowError({ code: "UNABLE_TO_RESOLVE_INTERNAL" });
      }
    }
  } else {
    ...
    if (!info.verified) {
      const verified = await verifyDomain(now, alias, {
        isExternal: info.isExternal
      });
      if (isNowError(verified)) {
        return verified;
      }
    }
...
type CreateAliasError =
  | NowError<"ALIAS_IN_USE", void>
  | NowError<"DEPLOYMENT_NOT_FOUND", { id: string }>
  | NowError<"INVALID_ALIAS", void>
  | NowError<"NEED_UPGRADE", void>
  | NowError<"NO_DOMAIN_PERMISSIONS", void>;
...
async function createAlias(
  ...
  deployment,
  alias
): Promise<AliasRecord | CreateAliasError> {
  ...
  try {
    ...
    if (error.code === "cert_missing" || error.code === "cert_expired") {
      ...
      return await createAlias(output, now, deployment, alias);
    }
    ...
    if (error.code === "invalid_alias") {
      return createNowError({ code: "INVALID_ALIAS" });
    }

    if (error.status === 403) {
      if (error.code === "custom_domain_needs_upgrade") {
        return createNowError({ code: "NEED_UPGRADE" });
      }

      if (error.code === "alias_in_use") {
        return createNowError({ code: "ALIAS_IN_USE" });
      }
...
): Promise<Deployment | null | FetchDeploymentErrors> {
...
async function deploymentIsAliased(
  now,
  deployment: Deployment
): Promise<boolean> {
...
function shouldDownscaleDeployment(
  now,
  deployment: NpmDeployment | BinaryDeployment
): boolean {
  return (
    !deploymentIsAliased(now, deployment) &&
  ...
  if (prevDeployment !== null && prevDeployment.type !== "STATIC") {
    if (shouldDownscaleDeployment(now, prevDeployment)) {
      await setScale(
        ...
        now,
        prevDeployment.uid,
        getDownscalePresets(prevDeployment)
      );
